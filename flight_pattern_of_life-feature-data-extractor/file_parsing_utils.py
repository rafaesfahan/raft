...
def create_csv_dict(dir_individual_flights, break_after = None):
    ...
    csv_or_parquet_files = list(glob.glob(dir_individual_flights + "/*.csv")) + list(glob.glob(dir_individual_flights + "/*.parquet"))
    if len(csv_or_parquet_files) >= 1:   # CASE: ONE FOLDER WITH .CSV OR .PARQUET FILES IN IT
        d = defaultdict(lambda: {})
        for file in csv_or_parquet_files:
            dummy_inner_dict = {"filler_key": file}
            filename = os.path.basename(file).split(".")[0]
            d[filename] = dummy_inner_dict

    else:                                # CASE: FOLDER WITH ANOTEHR FODLER WITH CSV OR PARQUET FILE #
        ...
        for flight_folder in flight_folders:
            ...
            for id_key in id_keys:
                ...
                csv_list = list(glob.glob(id_key_dir + "*.csv")) + list(glob.glob(id_key_dir + "*.parquet"))
                if len(csv_list) > 0:
